...
freeVarSet' :: Ignore -> Level -> Ctx -> Exp -> Set (Level,Ty)
freeVarSet' ignore threshold ctx expr = execState (go ctx expr) Set.empty where
  go :: Ctx -> Exp -> State (Set (Level,Ty)) ()
  go ctx expr = case expr of
    Loc' j            -> if j < threshold && not (Set.member j ignore)
...
freeVarMap' :: Ignore -> Level -> Ctx -> Exp -> (Map Level Level, Seq Ty)
freeVarMap' ignore threshold level expr = (tbl, tys) where
  set = freeVarSet' ignore threshold level expr
  lts = Set.toList set :: [(Level,Ty)]
  tbl = Map.fromList $ zip (map fst lts) [0..]
  tys = Seq.fromList $ map snd lts

{-
freeVarSet :: Ignore -> Ctx -> Exp -> Set (Level,Ty)
freeVarSet ignore ctx = freeVarSet' (ctxToLevel ignore ctx) ctx

freeVarMap :: Ignore -> Ctx -> Exp -> (Map Level Level, Seq Ty)
freeVarMap ignore ctx = freeVarMap' (ctxToLevel ignore ctx) ctx
-}
...
{-
-- we need to remember which let-bound variables where lifted to top-level
data CtxEntry 
  = TopEntry Ty TopLev     -- ^ this was a function introduced by a `let` which was lifted to top-level
  | ExpEntry Ty            -- ^ this was an expression introduced by a `let` 
  deriving (Eq,Show)

type LiftCtx = Ctx -- Seq CtxEntry
-}

type LiftCtx = Ctx 

lambdaLift :: Tm -> Program Exp
lambdaLift orig = case runState (go_ Set.empty emptyCtx orig) Seq.empty of { (main,tops) -> MkProgram tops main } where

  go_ :: Ignore -> LiftCtx -> Tm -> LiftM Exp
  go_ ignore ctx term = fromEither <$> goEither Nothing ignore ctx term
  ...
  goEither :: Maybe String -> Ignore -> LiftCtx -> Tm -> LiftM (Either Capture Exp)
  goEither mbName ignore ctx term = case term of
    Lam {}                   -> let name = maybe "lam" id mbName in
                                Left  <$> goLam    name ignore ctx term
    Log name term'@(Lam {})  -> Left  <$> goLam    name ignore ctx term'
    _                        -> Right <$> goNotLam      ignore ctx term
    where
  ...
  --
  goLam :: String -> Ignore -> LiftCtx -> Tm -> LiftM Capture
  goLam name ignore ctx term = case term of
    Lam {} -> {- debugNice "Lam/isLambda" (ctx,term) $ -} case isLambda' ctx term of
      MkLams (MkFunTy argTys retTy) body -> do
        ...
        body' <- go_ ignore ctx' body
        let level = ctxToLevel ctx
            nargs = length argTys
        let (free, freeTys) = freeVarMap' ignore level ctx' body'
  ...
  --
  goFix :: String -> Ignore -> LiftCtx -> Ty -> Tm -> LiftM Capture
  goFix name ignore ctx recTy term = case term of
    Log name' term'@(Lam {}) -> goFix name' ignore ctx recTy term'      -- naming hack
    Lam {} -> {- debugNice "Rec(Fix)/isLambda" (ctx,recTy,term) $ -} case isLambda' (ctx |> recTy) term of
      MkLams funTy@(MkFunTy argTys retTy) body -> do
        ...
        body' <- {- debug "type of rec" recTy $ -} go_ ignore ctx' body
        let level = ctxToLevel ctx
            nargs = length argTys
        let (free, freeTys) = freeVarMap' ignore level ctx' body'
            nfree = Seq.length freeTys
            argTys' =  debugNice "Rec/isLambda" (ctx,term,free,freeTys) $ F.toList freeTys ++ argTys
            -- nargs'  = nfree + nargs 
  ...
  goNotLam :: Ignore -> LiftCtx -> Tm -> LiftM Exp
  goNotLam ignore ctx term = case term of
    ...
    Let ty rhs body -> do
      let level = ctxToLevel ctx
      let ignore' = Set.insert level ignore
      rhsEi <- goEither (Just "fun") ignore   ctx        rhs      
      body' <- go_                   ignore' (ctx |> ty) body
      return $ case rhsEi of 
        Left  apps -> replaceVar (level :~> fromApps apps) body'
        Right rhs' -> Let' ty rhs' body'      

    -- if this was of the form `letrec f = \x y z -> ... f ... a ... b ... in ... f ...` 
    -- then appearances of `f` in the body needs to be replaced by `top a b`
    Rec ty rhs body -> case rhs of
      Lam {} -> do
        let level   = ctxToLevel ctx
        let ignore' = Set.insert level ignore
        apps  <- goFix "rec" ignore'  ctx    ty  rhs
        body' <- go_         ignore' (ctx |> ty) body
    ...
    App {} -> case isApp' term of
      MkApps fun args -> do
        fun'  <-       go_ ignore ctx  fun
        args' <- mapM (go_ ignore ctx) args
        return (App' fun' args')

    Pri op args -> Pri' op <$> mapM (go_ ignore ctx) args

    Lit k  -> return (Lit' k)

    Lam {} -> error "lambdaLift/goNotLam: shouldn't happen (Lam)"

    Dbg n t x y -> go_ ignore ctx y
    Log n     y -> go_ ignore ctx y
